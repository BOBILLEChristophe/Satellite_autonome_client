/*

  Signal.cpp


*/

#include "Signal.h"

uint8_t Signal::m_compt(0);
uint16_t Signal::m_data(0);
uint16_t Signal::m_masque(0);

Signal::Signal() : m_id(255),
                   m_type(0),
                   m_length(0),
                   m_decalage(0)
{}

void Signal::setup(const uint8_t x)
{
  m_decalage = m_compt;
  m_id = x;
  switch (m_type)
  {
  case 0: // Feu orange  - rouge - vert
    m_length = 3;
#ifdef DEBUG
    debug.print("Création d'un signal orange - vert - rouge n° ");
    debug.println(m_id);
#endif
    break;
  case 1: // Carré + oeilleton
    m_length = 5;
#ifdef DEBUG
    debug.print("Création d'un signal Carré + oeilleton n° ");
    debug.println(m_id);
#endif
    break;
  case 2: // Ralentissement (avec Carré + oeilleton)
    m_length = 7;
#ifdef DEBUG
    debug.print("Création d'un signal Ralentissement (avec Carré + oeilleton) n° ");
    debug.println(m_id);
#endif
    break;
  case 3 : // RRalentissement (avec Carré + oeilleton)
    m_length = 7;
#ifdef DEBUG
    debug.print("Création d'un signal RRalentissement (avec Carré + oeilleton) n° ");
    debug.println(m_id);
#endif
    break;
  }
  m_compt += m_length;
}

uint16_t Signal::affiche(uint16_t x)
{
  uint16_t clearMasque = 0;
  for (byte i = 0; i < m_length; i++)
    clearMasque += 1 << i;
  clearMasque = clearMasque << m_decalage;
  m_masque &= ~clearMasque;

  switch (m_type)
  {
  case 0:
    switch (x)
    {
    case 0: // orange
      m_data = 0x01 << m_decalage;
      break;
    case 1: // rouge
      m_data = 0x02 << m_decalage;
      break;
    case 2: // vert
      m_data = 0x04 << m_decalage;
      break;
    }
    break;
  case 2:
  case 3:
    switch (x)
    {
    case 0: // orange - avertissement
      m_data = B10 << m_decalage;
      break;
    case 1: // rouge - sémaphore
      m_data = B101 << m_decalage;
      break;
    case 2: // vert - voie libre
      m_data = B1000 << m_decalage;
      break;
    case 3: // rouge - carré
      m_data = B10100 << m_decalage;
      break;
    case 4: // ralentissement ou Rralentissement
      m_data = B1100000 << m_decalage;
      break;
    }
    break;
  }
  m_masque |= m_data;
  return m_masque;
}

void Signal::reset()
{
  m_compt = 0;
  m_data = 0;
}
void Signal::type(byte type) { m_type = type; }
byte Signal::type() { return m_type; }
void Signal::ID(const uint8_t id) { m_id = id; }